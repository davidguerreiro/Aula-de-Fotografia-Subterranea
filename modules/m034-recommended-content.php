<?php
/**
 * Module 034 - Recommended Content
 * 
 * @package aula/modules
 */

?>

<div class="module m34">
    <h2 class="m34__title">
        También te puede interesar ...
    </h2>

    <ul class="posts-list">
        <li>
            <article href="#" class="posts-list__post-link">
                <div class="posts-list__section posts-list__section--image-wrapper">
                    <img src="https://placebear.com/300/200" alt="">
                </div>
                <div class="posts-list__section">
                    <h3>Titulo del post</h3>
                    <ul class="posts-list__post-data-list">
                        <li>
                            <i class="fa fa-sun-o" aria-hidden="true"></i> Actividades
                        </li>
                        <li>
                            <i class="fa fa-pencil" aria-hidden="true"></i> Angel
                        </li>
                    </ul>
                    <p class="posts-list__excerpt">
                        Esta cueva estaba vacia tuvimos que irnos para casa
                    </p>
                    <a href="#" class="posts-list__read-more">
                        Leer más
                        <i class="fa fa-angle-right" aria-hidden="true"></i>
                    </a>
                </div>
            </article>
        </li>

        <li>
            <article href="#" class="posts-list__post-link">
                <div class="posts-list__section posts-list__section--image-wrapper">
                    <img src="https://placebear.com/300/200" alt="">
                </div>
                <div class="posts-list__section">
                    <h3>Titulo del post</h3>
                    <ul class="posts-list__post-data-list">
                        <li>
                            <i class="fa fa-sun-o" aria-hidden="true"></i> Actividades
                        </li>
                        <li>
                            <i class="fa fa-pencil" aria-hidden="true"></i> Angel
                        </li>
                    </ul>
                    <p class="posts-list__excerpt">
                        Esta cueva estaba vacia tuvimos que irnos para casa
                    </p>
                    <a href="#" class="posts-list__read-more">
                        Leer más
                        <i class="fa fa-angle-right" aria-hidden="true"></i>
                    </a>
                </div>
            </article>
        </li>

        <li>
            <article href="#" class="posts-list__post-link">
                <div class="posts-list__section posts-list__section--image-wrapper">
                    <img src="https://placebear.com/300/200" alt="">
                </div>
                <div class="posts-list__section">
                    <h3>Titulo del post</h3>
                    <ul class="posts-list__post-data-list">
                        <li>
                            <i class="fa fa-sun-o" aria-hidden="true"></i> Actividades
                        </li>
                        <li>
                            <i class="fa fa-pencil" aria-hidden="true"></i> Angel
                        </li>
                    </ul>
                    <p class="posts-list__excerpt">
                        Esta cueva estaba vacia tuvimos que irnos para casa
                    </p>
                    <a href="#" class="posts-list__read-more">
                        Leer más
                        <i class="fa fa-angle-right" aria-hidden="true"></i>
                    </a>
                </div>
            </article>
        </li>
    </ul>
</div>